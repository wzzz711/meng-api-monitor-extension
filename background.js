...
class XHRMonitorBackground {
  ...
  async storeRequest(requestData, tabId) {
    ...
    // 存储更新后的请求列表
    //
    // 此处选择为每个标签页（tabId）使用独立的 key 进行存储，而不是将所有标签页的数据聚合在一个大的对象下（例如 "allRequests": { "tab1": [...], "tab2": [...] }），是因为：
    //
    // 1. 避免性能瓶颈: 如果所有请求都在一个键下，每次为一个标签页添加一条新记录时，都需要完整读写整个庞大的数据对象，效率极低。
    // 2. 防止竞争条件: `chrome.storage` 的读写非原子性。如果多个标签页近乎同时更新，后一次的写操作会覆盖前一次的更新，导致数据丢失。
    //
    // 分布式存储相较于集中式的存储，确保了每个标签页的写操作都是独立的，保证了高性能和数据一致性。
